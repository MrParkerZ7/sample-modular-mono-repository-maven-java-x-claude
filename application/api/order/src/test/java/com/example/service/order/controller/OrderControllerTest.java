...
import com.example.service.order.config.SecurityConfig;
...
class OrderControllerTest {
  ...
  void testUpdateOrderStatus() throws Exception {
    ...
    mockMvc
        .perform(put("/api/orders/order-123/status/CONFIRMED").with(jwt()))
        .andExpect(status().isOk())
  ...
  void testUpdateOrderStatusNotFound() throws Exception {
    ...
    mockMvc
        .perform(put("/api/orders/non-existent/status/CONFIRMED").with(jwt()))
        .andExpect(status().isNotFound());
  }
  ...
  void testCancelOrder() throws Exception {
    ...
    mockMvc.perform(delete("/api/orders/order-123").with(jwt())).andExpect(status().isNoContent());
  }
  ...
  void testCancelOrderNotFound() throws Exception {
    ...
    mockMvc
        .perform(delete("/api/orders/non-existent").with(jwt()))
        .andExpect(status().isNotFound());
  }
}
