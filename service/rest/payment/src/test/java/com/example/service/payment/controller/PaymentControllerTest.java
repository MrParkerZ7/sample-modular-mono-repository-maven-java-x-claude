...
class PaymentControllerTest {
  ...
  void testRefundPayment() throws Exception {
    ...
    mockMvc
        .perform(post("/api/payments/pay-123/refund"))
        .andExpect(status().isOk())
  ...
  void testRefundPaymentNotFound() throws Exception {
    ...
    mockMvc.perform(post("/api/payments/non-existent/refund")).andExpect(status().isNotFound());
  }
  ...
  void testCancelPayment() throws Exception {
    ...
    mockMvc.perform(delete("/api/payments/pay-123")).andExpect(status().isNoContent());
  }
  ...
  void testCancelPaymentNotFound() throws Exception {
    ...
    mockMvc.perform(delete("/api/payments/non-existent")).andExpect(status().isNotFound());
  }
}
